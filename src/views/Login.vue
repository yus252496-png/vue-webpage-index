...
export default {
  ...
  computed: {
    pageStyle() {
      return (this.$store && this.$store.getters && this.$store.getters.pageStyle) || 'cyberpunk'
    }
  },
  mounted() {
  ...
  methods: {
    getThemeColors() {
      const theme = this.pageStyle
      const palettes = {
        cyberpunk: {
          background: '#0a0a0f',
          grid: 'rgba(0, 243, 255, 0.1)',
          textColors: ['#00f3ff', '#ff00ff'],
          interference: '255, 0, 255',
          scanline: 'rgba(0, 243, 255, 0.1)'
        },
        minimal: {
          background: '#f8f9fa',
          grid: 'rgba(0,0,0,0.05)',
          textColors: ['#007bff', '#6c757d'],
          interference: '108, 117, 125',
          scanline: 'rgba(0,0,0,0.04)'
        },
        classic: {
          background: '#f5f0e8',
          grid: 'rgba(139,69,19,0.06)',
          textColors: ['#8b4513', '#a0522d'],
          interference: '139, 69, 19',
          scanline: 'rgba(139,69,19,0.06)'
        },
        modern: {
          background: '#f5f7fa',
          grid: 'rgba(64,158,255,0.06)',
          textColors: ['#409eff', '#67c23a'],
          interference: '64, 158, 255',
          scanline: 'rgba(64,158,255,0.06)'
        },
        comic: {
          background: '#fdf6e3',
          grid: 'rgba(0,0,0,0.06)',
          textColors: ['#d4760a', '#e74c3c'],
          interference: '212, 118, 10',
          scanline: 'rgba(0,0,0,0.05)'
        },
        sketch: {
          background: '#f0f0f0',
          grid: 'rgba(0,0,0,0.07)',
          textColors: ['#34495e', '#2c3e50'],
          interference: '52, 73, 94',
          scanline: 'rgba(0,0,0,0.05)'
        }
      }
    ...
    generateCaptcha() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        ctx.strokeStyle = `rgba(${colors.interference}, ${Math.random() * 0.5 + 0.2})`
        ctx.lineWidth = 1
        ctx.stroke()
      }
      ...
      for (let i = 0; i < code.length; i++) {
        ...
        ctx.shadowColor = colors.textColors[i % 2]
        ctx.shadowBlur = 10
        ctx.fillStyle = colors.textColors[i % 2]
